...
class Animation 
{
    ...
    */
    ...
    //Format des données : "couleur_de_la_led/nombre_de_led_de_cette_couleur/numéro_de_la_1e_led/.../numéro_de_la_N_ème_led"
    export() {
        let data = [];
	let ma_var = "var_tankin=";

            for (const color of Object.keys(this.colors[0]))
            {
		let ledsWithThisColor = this.getLedsWithColor(0, color);

                ma_var += color + "/";
		ma_var += ledsWithThisColor.length;
	...
	//let ma_var = "var_tankin=zoo";

        xhr.open("POST", "write_led_file.php", true);
	xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xhr.send(ma_var); //data
        //fin envoie POST
	console.log(this);
